...
function print_table($table, $order)
{
	$result = pg_query("select * from $table order by $order;");
	$is_first_row = true;
	echo "<table border>";
	while ($row = pg_fetch_assoc($result))
	{
...
if (isset($_GET['mode']))
{
	if ($_GET['mode'] == "oper") print_table("oper","name");
	if ($_GET['mode'] == "dolg") print_table("dolg","name");
	if ($_GET['mode'] == "podr") print_table("podr","name");
	if ($_GET['mode'] == "fact") print_table("fact","name");
	if ($_GET['mode'] == "xpoh") print_table("xpoh","fio");
} else {
	print_frv();
}
?>
